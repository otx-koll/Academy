...
if (count > 0) {
	forumList = forumDao.getForums(startRow, pageSize);
...
<table border="1">
	...
	%>
		<tr>
			<td><%=forumVo.getNum() %></td>
			<td class="left">
				<%
				if (forumVo.getReLev() > 0) { // 답글이면
					%>
					<img src="/images/center/level.gif" width="<%=forumVo.getReLev() * 15 %>" height="13">
					<img src="/images/center/re.gif">
					<%
				}
				%>
				<a href="content.jsp?num=<%=forumVo.getNum() %>&pageNum=<%=pageNum %>"><%=forumVo.getSubject() %></a>
			</td>
			<td><%=forumVo.getId() %></td>
			<td><%=sdf.format(forumVo.getRegDate()) %></td>
			<td><%=forumVo.getReadcount() %></td>
			<td><%=forumVo.getThumbsUp() %></td>
		</tr>
		<%
			}
		} else { // count == 0
